...
/**
 * Created by jcapiz on 30/04/16.
 */
public class ObdMockService extends Service {
    ...
    public class LocalBinder extends Binder {
        public ObdMockService getService() {
            return ObdMockService.this;
        }
    }
    ...
    private final class ServiceHandler extends Handler {
        ...
        private void runCommand(final ObdCommand cmd) {
            try {
                ...
                Log.d("Melchor",cmd.getName() + ": 48");
                TripsData td = new TripsData(ObdMockService.this);
                ...
                if (cmd.getName().equals(AvailableCommandNames.SPEED.getValue())) {
                    if(mActivity != null ) mActivity.runOnUiThread(
                            new Runnable() {
                                @Override
                                public void run() {
                                    ((MainActivity)mActivity).setSpeedText("48");
                                }
                            }
                    );
                    int lrid = td.insertaVelocidad("48", time, trip.getIdTrip());
                    value = new Speed(lrid, "48", time, trip.getIdTrip());
                } else if (cmd.getName().equals(AvailableCommandNames.ENGINE_RPM.getValue())) {
                    if(mActivity != null ) mActivity.runOnUiThread(
                            new Runnable() {
                                @Override
                                public void run() {
                                    ((MainActivity)mActivity).setRpmText("48");
                                }
                            }
                    );
                    int lrid = td.insertaRPM("54", time, trip.getIdTrip());
                    value = new RPM(lrid, "54", time, trip.getIdTrip());
                } else if (cmd.getName().equals(AvailableCommandNames.THROTTLE_POS.getValue())) {
                    if(mActivity != null ) mActivity.runOnUiThread(
                            new Runnable() {
                                @Override
                                public void run() {
                                    ((MainActivity)mActivity).setPdaText("16%");
                                }
                            }
                    );
                    int lrid = td.insertaThrottlePos("16%", time, trip.getIdTrip());
                    value = new ThrottlePos(lrid, "16%", time, trip.getIdTrip());
                }
                if (value != null) {
                    Uploader up = new Uploader(value);
                    up.setContext(ObdMockService.this);
                    up.start();
                }
    ...
    public void setActivity(Activity mActivity){
        ...
        if (permissionCheck == PackageManager.PERMISSION_GRANTED) {
            ...
            mActivity.runOnUiThread(new Runnable(){
                @Override
                public void run(){
                    ((MainActivity)ObdMockService.this.mActivity).makeSnackbar("Necesitamos permiso de " +
                            "ubicación");
                }
            });
        }
