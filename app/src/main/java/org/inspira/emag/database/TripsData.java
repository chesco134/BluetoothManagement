...
public class TripsData extends SQLiteOpenHelper{
	...
	public void setCommited(String table, int idTrip){
		Log.d("XXXXXXXXXXXXXX", "Commiting " + table + ", " + idTrip);
	...
	public int insertTrip(Date fechaInicio){
	...
	public Speed[] getSpeedsByTrip(int idTrip){
		...
		while(c.moveToNext()){
			speeds.add(new Speed(c.getInt(c.getColumnIndex("idValue")),c.getString(c.getColumnIndex("RPMVal")),c.getString(c.getColumnIndex("Timestamp")),c.getInt(c.getColumnIndex("idTrip"))));
		}
		c.close();
		close();
